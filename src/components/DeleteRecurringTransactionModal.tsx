...
interface DeleteRecurringTransactionModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (recurringId: string) => void;
}
      ...
      <div className="relative w-full max-w-md mx-4 bg-white/90 backdrop-blur-xl border border-white/40 shadow-2xl rounded-xl p-8 flex flex-col gap-6 overflow-hidden">
        ...
        <div className="flex items-start justify-between relative z-10">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 text-[#ba1a1a]">
              ...
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">Delete Automation</h2>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4 relative z-10">
          <p className="text-slate-600 leading-relaxed font-normal text-sm">
            Are you sure you want to delete this recurring transaction? This action is permanent and cannot be undone.
          </p>

          {/* Context Card (Selected Recurring Transaction Preview) */}
          <div className="bg-slate-50/80 border border-slate-200/60 rounded-lg p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold text-slate-700 truncate">{recurringTransaction.description}</div>
                <div className="text-xs text-slate-500 truncate capitalize">{recurringTransaction.category} • {recurringTransaction.frequency}</div>
              </div>
            </div>
            <div className={`text-sm font-bold ml-4 whitespace-nowrap ${recurringTransaction.type === 'income' ? 'text-emerald-600' : 'text-slate-900'}`}>
              {recurringTransaction.type === 'income' ? '+' : '-'}
              {formatCurrency(Math.abs(parseFloat(recurringTransaction.amount.toString())), recurringTransaction.currency)}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-2 relative z-10">
          ...
          <button
            onClick={() => {
              onConfirm(recurringTransaction.id);
              onClose();
            }}
